...
if (isset($_POST['UpdatePass'])) {
    ...
    $pass_baru = mysqli_real_escape_string($conn, $_POST['pswd2']);
    $konfirmasi_pass_baru = mysqli_real_escape_string($conn, $_POST['pswd3']);

    $querypass = mysqli_query($conn, "SELECT * FROM kasir WHERE password='$pass1'");
    $caripass = mysqli_fetch_assoc($querypass);

    $validasi_password = $pass_baru === $konfirmasi_pass_baru;

    if ($validasi_password) {
        $sql = "UPDATE kasir SET password = '$pass_baru' WHERE userid = {$caripass['userid']}";
